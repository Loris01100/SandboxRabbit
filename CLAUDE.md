...
- `life` sert de vie pour les gaz, de compteur de chute pour `NITRO`, d'amorçage pour `C4`, de ticks de combustion pour `THERMITE`, de charge utile pour `SOURCE` (la matière émise), de mèche allumée pour `CANDLE` et de repos pour `METAL` : ne pas le réinitialiser à l'aveugle. C'est un `Uint8Array`, donc `life` ≤ 250 dans `MATERIALS`.
- Un explosif se distingue par son **déclencheur**, pas par son rayon : feu (`TNT`), choc (`NITRO`), étincelle (`C4`), poids (`MINE`), volume (`FIREDAMP` via `flammable`). Ajouter un explosif de plus = ajouter un déclencheur, sinon c'est du TNT repeint.
